    ...
    <div class="card-body drawer-body" id="kt_drawer_chat_messenger_body">
        ...
        <div class="scroll-y me-n5 pe-5" data-kt-element="messages" data-kt-scroll="true" data-kt-scroll-activate="true"
            ...
            <div class="fv-row mb-8">
                ...
                <input type="text" name="employee_ad_id" placeholder="Employee Ad ID" id="employee_ad_id"  value="{{ $row->employee_ad_id }}"
                    value="" class="form-control" required />
            </div>
            <div class="fv-row mb-8">
                <label class="required fs-6 fw-semibold mb-2">Full Name</label>
                <input type="text" name="full_name" id="full_name" placeholder="Enter full name" value="{{ $row->full_name }}"
                    class="form-control" required />
            </div>
            <div class="fv-row mb-8">
                <label class="required fs-6 fw-semibold mb-2">Employee Email</label>
                <input type="text" name="email_address" placeholder="Enter email" id="email_address" value="{{ $row->email_address }}"
                    class="form-control" required />
            </div>
            <div class="fv-row mb-8">
                ...
                <select class="form-select form-select-solid" data-control="select2" data-hide-search="false"
                    ...
                    @php $selected ='';@endphp
                        ...
                        @php $selected =($dep->id == $row->department_id)?'selected':'';@endphp
            ...
            {{-- <div class="fv-row mb-8">
                <label class="fs-6 fw-semibold mb-2">Teams</label>
                <select class="form-select form-select-solid" data-control="select2" data-hide-search="false" multiple
                    data-placeholder="Select a Team" name="team[]">
                    @if ($teams)
                         @php $selected ='';@endphp
                        @foreach ($teams as $team)
                        
                        @php $selected =(in_array($row->employee_ad_id,$team->getTeamEmployeeIds()))?'selected':'';@endphp
