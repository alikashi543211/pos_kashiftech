        ...
        <div class="card mb-5 mb-xl-8">
            ...
            <div class="card-header border-0 pt-5">
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" data-user-listing-table-filter="search" class="form-control form-control-solid w-250px ps-13" placeholder="Search Live Link" />
                </div>
                <div class="card-toolbar">
                    @if (validatePermissions('resume/live-link-sections/add'))
                        <a href="javascript:void(0)" class="btn btn-sm btn-primary float-end me-3 btn-add"><i class="fa-solid fa-plus"></i> Add New Live Link</a>
                    @endif
                </div>
            </div>
            <!--end::Header-->
                    ...
                    <table id="datatableusr" class="table table-hover table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                        <thead>
                            <tr class="fw-bold text-muted">
                                <th class="min-w-150px">Link Thumbnail</th>
                                <th class="min-w-150px">Link Name</th>
                                <th class="min-w-120px">Live Link</th>
                        ...
                        <tbody>
                            @if ($live_links)
                            @foreach ($live_links as $row)

                            <tr>
                                <td>
                                    <img src="{{ $row->link_thumbnail }}" width="150" height="50" alt="">
                                </td>
                                <td>
                                    {{ $row->link_name ?? 0 }}
                                </td>
                                <td>
                                    <span style="word-break: break-all;">{{ $row->live_link ?? 0 }}</span>
                                </td>

                                <td>
                                    <span title="Change Skill Status" class="badge {{ $row->is_active  == 1 ? 'badge-light-success active-status' : 'badge-light-danger inactive-status' }}" data-uid="{{ $row->id }}" style="cursor: pointer;">
                                        {{ $row->is_active  == 1 ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td>
                                    <input data-live-link-id="{{ $row->id }}" type="text" class="form-control text-center number_input_only sort_number_input" style="width: 50px;" value="{{ $row->sort_number ?? '' }}">
                                </td>
...
<script src="{{ asset('/assets/admin') }}/resume_live_link_sections.js?v={{ time() }}"></script>
@stop
