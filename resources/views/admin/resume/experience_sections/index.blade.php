        ...
        <div class="card mb-5 mb-xl-8">
            ...
            <div class="card-header border-0 pt-5">
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" data-user-listing-table-filter="search" class="form-control form-control-solid w-250px ps-13" placeholder="Search experience" />
                </div>
                <div class="card-toolbar">
                    @if (validatePermissions('resume/experience-sections/add'))
                        <a href="javascript:void(0)" class="btn btn-sm btn-primary float-end me-3 btn-add"><i class="fa-solid fa-plus"></i> Add New Experience</a>
                    @endif
                </div>
            </div>
            <!--end::Header-->
                    ...
                    <table id="datatableusr" class="table table-hover table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                        <thead>
                            <tr class="fw-bold text-muted">
                                <th class="min-w-150px">Company Logo</th>
                                <th class="min-w-150px">Company Name</th>
                                <th class="min-w-120px">Designation Name</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <img src="{{ $row->company_logo }}" width="60" height="50" alt="">
                                </td>
                                <td>
                                    {{ $row->company_name ?? '' }}
                                </td>
                                ...
                                <td>
                                    <span title="Change Experience Status" class="badge {{ $row->is_active  == 1 ? 'badge-light-success active-status' : 'badge-light-danger inactive-status' }}" data-uid="{{ $row->id }}" style="cursor: pointer;">
                                        {{ $row->is_active  == 1 ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td>
                                    <input data-experience-id="{{ $row->id }}" type="text" class="form-control text-center number_input_only sort_number_input" style="width: 50px;" value="{{ $row->sort_number ?? '' }}">
                                </td>
                                <td class="text-center">
                                    @if(validatePermissions('resume/experience-sections/descriptions/add/{id}'))
                                    <a title="Add Multiple Descriptions" href="javascript:void(0)" data-id="{{ $row->id }}" class="btn-add-descriptions btn-active-color-primary btn-sm me-1">
...
<script src="{{ asset('/assets/admin') }}/resume_experience_sections.js?v={{ time() }}"></script>
@stop
