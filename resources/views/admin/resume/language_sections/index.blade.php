        ...
        <div class="card mb-5 mb-xl-8">
            ...
            <div class="card-header border-0 pt-5">
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" data-user-listing-table-filter="search" class="form-control form-control-solid w-250px ps-13" placeholder="Search language" />
                </div>
                <div class="card-toolbar">
                    @if (validatePermissions('resume/language-sections/add'))
                        <a href="javascript:void(0)" class="btn btn-sm btn-primary float-end me-3 btn-add"><i class="fa-solid fa-plus"></i> Add New Language</a>
                    @endif
                </div>
            </div>
            <!--end::Header-->
                    ...
                    <table id="datatableusr" class="table table-hover table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                        <thead>
                            <tr class="fw-bold text-muted">
                                <th class="min-w-150px">Language</th>
                        ...
                        <tbody>
                            @if ($resumeLanguages)
                            @foreach ($resumeLanguages as $row)
                            ...
                            <tr>
                                ...
                                <td>
                                    {!! renderStars($row->language_rating ?? 0) !!}
                                </td>

                                <td>
                                    <span title="Change Language Status" class="badge {{ $row->is_active  == 1 ? 'badge-light-success active-status' : 'badge-light-danger inactive-status' }}" data-uid="{{ $row->id }}" style="cursor: pointer;">
                                        {{ $row->is_active  == 1 ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td>
                                    <input data-language-id="{{ $row->id }}" type="text" class="form-control text-center number_input_only sort_number_input" style="width: 50px;" value="{{ $row->sort_number ?? '' }}">
                                </td>
...
<script src="{{ asset('/assets/admin') }}/resume_language_sections.js?v={{ time() }}"></script>
@stop
