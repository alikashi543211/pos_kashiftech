                ...
                <div class="py-3 card-body">
                    <form name="resume_header_sections" enctype="multipart/form-data" id="resume_header_sections_form"
                        ...
                        <div class="mb-8 col-md-12">
                            <label class="mb-2 required fs-6 fw-semibold">Download Resume (Show/Hide)</label>
                            <select name="resume_about_section_download_resume_selection"
                                id="resume_about_section_download_resume_selection" class="form-control" required>
                                <option value="" disabled selected>Select Option</option>
                                <option value="yes"
                                    {{ ($headerSection->resume_about_section_download_resume_selection ?? '') == 'yes' ? 'selected' : '' }}>
                                    Show</option>
                                <option value="no"
                                    {{ ($headerSection->resume_about_section_download_resume_selection ?? '') == 'no' ? 'selected' : '' }}>
                                    Hide</option>
                            </select>
                        </div>
                        <div class="mb-8 col-md-12">
                            <label class="mb-2 required fs-6 fw-semibold">About Section Text</label>
                            <textarea type="text" name="about_section_text" id="about_section_text"
                                placeholder="Enter resume section description" class="form-control" required>{{ $headerSection->about_section_text ?? '' }}</textarea>
                        </div>
                    </form>
                    <!--end::Table container-->
                </div>
