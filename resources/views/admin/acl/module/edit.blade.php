<form name="fform" enctype="multipart/form-data" id="editForm" method="post" action="">
    @csrf
    <div class="card-body drawer-body" id="kt_drawer_chat_messenger_body">
        ...
        <div class="scroll-y me-n5 pe-5" data-kt-element="messages" data-kt-scroll="true" data-kt-scroll-activate="true"
                ...
                <select class="form-select form-select-solid" data-control="select2" data-hide-search="false"
                    ...
                    @if ($catResult)
                        @foreach ($catResult as $rowCat)
                            @php
                                $selected = $rowCat->ID == $row->module_category_ID ? 'selected' : '';
                            @endphp
                            <option value="{{ $rowCat->ID }}" {{ $selected }}>{{ $rowCat->category_name }}
                            </option>
                        @endforeach
                    @endif
                </select>
            ...
            <div class="fv-row mb-8">
                ...
                <input type="text" name="module_name" placeholder="Module Name" value="{{ $row->module_name }}"
                    id="module_name" class="form-control" required />
            </div>
            <div class="fv-row mb-8">
                <label class="required fs-6 fw-semibold mb-2">Slug</label>
                <input type="text" name="route" id="route" value="{{ $row->route }}" placeholder="Enter Slug"
                     class="form-control" required />
            </div>
            <div class="fv-row mb-8">
                <label class="fs-6 fw-semibold mb-2">CSS Class</label>
                <input type="text" name="css_class" placeholder="Enter CSS Class" value="{{ $row->css_class }}"
                    id="css_class" class="form-control" />
            </div>
            <div class="fv-row mb-8">
                <label class="fs-6 fw-semibold mb-2">Show in Header</label>
                <input type="checkbox" name="show_in_menu_checkbox" id="show_in_menu_checkbox" value="{{ $row->show_in_menu }}"
                class="form-control form-check-input header-module" {{ $row->show_in_menu ? 'checked' : '' }}
                onChange="updateShowInMenuValue()"
                    />
            </div>
