$(document).ready(function () {
    ...
    $(document).on("click", ".btn-add", function (e) {
        $(".modal-body").html("");
        $(".drawer-title").html("Add New Department");
        jQuery.getJSON(admin_url + "/department/add/", function (response) {
    ...
    $(document).on("click", ".btn-edit", function (e) {
        $(".modal-body").html("");
        $(".drawer-title").html("Edit Department");
        var id = $(this).data("id");
        console.log('id:', id)
        jQuery.getJSON(
            admin_url + "/department/edit/" +id,
            function (response) {
                if (response.responseCode == 1) {
                    $(".drawer-body").html(response.html);
                    const drawerElement = document.querySelector("#kt_drawer_chat");
    ...
    $(document).on("click", ".btn-status-change", function (e) {
        ...
        var message = 'Are you sure you want to change Department status?';
        var statusElement = $(this);
        var updateFundsStatusUrl = admin_url + "/department-update-status";
    ...
    $(document).on("click", ".btn-del", function(e) {
        ...
        var deleteUrl = admin_url + "/department/delete";
        Swal.fire({
            title: 'Are you sure?',
    ...
    //Form 
       ...
       $(document).on("submit", "#fform", function (e) {
        ...
        // loadSpinner();
        $.ajax({
            url: admin_url + "/department/" + params,
            type: "POST",
    ...
    var table = $('#department-listing-table').DataTable({
        info: false,
        order: [],
        pageLength: 50,
        lengthChange: false,
    ...
    document.querySelector('[data-department-listing-table-filter="search"]').addEventListener("keyup", function (t) {
