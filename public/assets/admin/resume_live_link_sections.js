$(document).ready(function () {
    ...
    $(document).on("click", ".btn-add", function (e) {
        console.log("e:", e);
        $(".modal-body").html("");
        $(".drawer-title").html("Add New Live Link");
        jQuery.getJSON(admin_url + "/resume/live-link-sections/add", function (response) {
    ...
    $(document).on("click", ".btn-edit", function (e) {
        $(".modal-body").html("");
        $(".drawer-title").html("Edit Existing Live Link");
        var id = $(this).data("id");
        console.log("id:", id);
        jQuery.getJSON(admin_url + "/resume/live-link-sections/edit/" + id, function (response) {
            if (response.responseCode == 1) {
                $(".drawer-body").html(response.html);
                const drawerElement = document.querySelector("#kt_drawer_chat");
                if (drawerElement) {
    ...
    $(document).on("click", ".btn-status-change", function (e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: admin_url + "/resume/live-link-sections/status-change/" + did,
                    method: "GET",
    ...
    $(document).on("click", ".btn-delete", function (e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: admin_url + "/resume/live-link-sections/delete/" + did,
                    method: "GET",
                    success: function (data) {
                        var obj = JSON.parse(data);
                        if (obj.responseCode == 1) {
    ...
    $(document).on("submit", "#fform", function (e) {
        ...
        var eid = $("#eid").val();
        var params = act_val;
        var dynamic_url = "/resume/live-link-sections/add";
        if (eid != "") {
            dynamic_url = "/resume/live-link-sections/" + act_val + "/" + eid;
        }
        // loadSpinner();
        $.ajax({
...
$(document).on('blur', '.sort_number_input', function(){
    ...
    $.ajax({
        url: admin_url + "/resume/live-link-sections/sorting/" + row_id + "/" + sort_number,
        method: "GET",
        success: function (data) {
            var obj = JSON.parse(data);
            if (obj.responseCode == 1) {
