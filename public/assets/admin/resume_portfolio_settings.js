$(document).ready(function () {
    ...
    $(document).on("submit", "#resume_header_sections_form", function (e) {
        ...
        // loadSpinner();
        $.ajax({
            url: admin_url + "/resume/portfolio-settings",
            type: "POST",
            data: new FormData(this),
...
function readAndPreviewImage(input, previewContainer) {
    if (input.files && input.files[0]) {
        let reader = new FileReader();
        reader.onload = function (e) {
            let imgPreview = previewContainer.find("img");
...
$(document).ready(function () {
    $(".portfolio_setting_image_file").change(function () {
        let previewContainer = $(this).closest(".images-block").find(".d-flex"); // Ensure correct preview container
        readAndPreviewImage(this, previewContainer);
    });
});
