$(document).ready(function () {
    ...
    $(document).on("click", ".btn-add", function (e) {
        console.log("e:", e);
        $(".modal-body").html("");
        $(".drawer-title").html("Add New Service");
        jQuery.getJSON(admin_url + "/resume/service-sections/add", function (response) {
    ...
    $(document).on("click", ".btn-add-descriptions", function (e) {
        ...
        $(".drawer-title").html("Add Service Descriptions");
        jQuery.getJSON(admin_url + "/resume/service-sections/descriptions/add/" + project_id, function (response) {
            if (response.responseCode == 1) {
                $(".drawer-body").html(response.html);
                const drawerElement = document.querySelector("#kt_drawer_chat");
                if (drawerElement) {
    ...
    $(document).on("click", ".btn-add-images", function (e) {
        ...
        $(".modal-body").html("");
        var project_id = $(this).attr('data-id');
        $(".drawer-title").html("Add Service Images");
        jQuery.getJSON(admin_url + "/resume/service-sections/images/add/" + project_id, function (response) {
            if (response.responseCode == 1) {
                $(".drawer-body").html(response.html);
                const drawerElement = document.querySelector("#kt_drawer_chat");
                if (drawerElement) {
    ...
    $(document).on("click", ".btn-edit", function (e) {
        $(".modal-body").html("");
        $(".drawer-title").html("Edit Existing Service");
        var id = $(this).data("id");
        console.log("id:", id);
        jQuery.getJSON(admin_url + "/resume/service-sections/edit/" + id, function (response) {
            if (response.responseCode == 1) {
                $(".drawer-body").html(response.html);
                const drawerElement = document.querySelector("#kt_drawer_chat");
                if (drawerElement) {
    ...
    $(document).on("click", ".delete-image-btn", function (e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    url: admin_url + "/resume/service-sections/images/delete/" + did,
                    method: "GET",
    ...
    $(document).on("click", ".btn-status-change", function (e) {
        ...
        Swal.fire({
            title: "Confirmation",
            text: "You want to change Service status.",
            icon: "warning",
            animation: !1,
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            ...
            if (result.value) {
                $.ajax({
                    url: admin_url + "/resume/service-sections/status-change/" + did,
                    method: "GET",
                    success: function (data) {
                        var obj = JSON.parse(data);
                        if (obj.responseCode == 1) {
    ...
    $(document).on('click', '.active-status, .inactive-status', function (e) {
        ...
        var confirmText = 'Are you sure you want to ' + action.toLowerCase() + ' this service?';
        var $badge = $(this);
        Swal.fire({
            title: 'Confirm',
            text: confirmText,
    ...
    $(document).on("click", ".btn-delete", function (e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: admin_url + "/resume/service-sections/delete/" + did,
                    method: "GET",
                    success: function (data) {
                        var obj = JSON.parse(data);
                        if (obj.responseCode == 1) {
    ...
    $(document).on("submit", "#fform", function (e) {
        ...
        var eid = $("#eid").val();
        var params = act_val;
        var dynamic_url = "/resume/service-sections/add";
        if (eid != "") {
            dynamic_url = "/resume/service-sections/" + act_val + "/" + eid;
        }
        // loadSpinner();
        $.ajax({
    ...
    $(document).on("submit", "#project_section_add_images_form", function (e) {
        ...
        var eid = $("#eid").val();
        var params = act_val;
        var dynamic_url = "/resume/service-sections/images/add/" + project_id;
        // loadSpinner();
        $.ajax({
            url: admin_url + dynamic_url,
            type: "POST",
    ...
    $(document).on("submit", "#project_section_add_descriptions_form", function (e) {
        ...
        var eid = $("#eid").val();
        var params = act_val;
        var dynamic_url = "/resume/service-sections/descriptions/add/" + project_id;
        // if (eid != "") {
        //     dynamic_url = "/resume/service-sections/" + act_val + "/" + eid;
        // }
        // loadSpinner();
...
$(document).on('blur', '.sort_number_input', function () {
    ...
    $.ajax({
        url: admin_url + "/resume/service-sections/sorting/" + row_id + "/" + sort_number,
        method: "GET",
        success: function (data) {
            var obj = JSON.parse(data);
            if (obj.responseCode == 1) {
