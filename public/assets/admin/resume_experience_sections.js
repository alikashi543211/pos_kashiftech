$(document).ready(function () {
    ...
    $(document).on("click", ".btn-add", function (e) {
        console.log("e:", e);
        $(".modal-body").html("");
        $(".drawer-title").html("Add New Experience");
        jQuery.getJSON(admin_url + "/resume/experience-sections/add", function (response) {
    ...
    $(document).on("click", ".btn-edit", function (e) {
        $(".modal-body").html("");
        $(".drawer-title").html("Edit Existing Experience");
        var id = $(this).data("id");
        console.log("id:", id);
        jQuery.getJSON(admin_url + "/resume/experience-sections/edit/" + id, function (response) {
            if (response.responseCode == 1) {
                $(".drawer-body").html(response.html);
                const drawerElement = document.querySelector("#kt_drawer_chat");
                if (drawerElement) {
    ...
    $(document).on("click", ".btn-status-change", function (e) {
        ...
        Swal.fire({
            title: "Confirmation",
            text: "You want to change experience status.",
            icon: "warning",
            animation: !1,
            ...
            if (result.value) {
                $.ajax({
                    url: admin_url + "/resume/experience-sections/status-change/" + did,
                    method: "GET",
    ...
    $(document).on('click', '.active-status, .inactive-status', function(e) {
        ...
        var confirmText = 'Are you sure you want to ' + action.toLowerCase() + ' this experience?';
        var $badge = $(this);
        Swal.fire({
            title: 'Confirm',
            text: confirmText,
            ...
            if (result.value) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire(
                            'Updated!',
                            'experience status has been updated.',
                            'success'
                        );
                    },
    ...
    $(document).on("click", ".btn-delete", function (e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: admin_url + "/resume/experience-sections/delete/" + did,
                    method: "GET",
                    success: function (data) {
                        var obj = JSON.parse(data);
                        if (obj.responseCode == 1) {
    ...
    $(document).on("submit", "#fform", function (e) {
        ...
        var eid = $("#eid").val();
        var params = act_val;
        var dynamic_url = "/resume/experience-sections/add";
        if (eid != "") {
            dynamic_url = "/resume/experience-sections/" + act_val + "/" + eid;
        }
        // loadSpinner();
        $.ajax({
    ...
    $(document).on("click", ".btn-add-descriptions", function (e) {
        ...
        $(".drawer-title").html("Add Experience Descriptions");
        jQuery.getJSON(admin_url + "/resume/experience-sections/descriptions/add/" + project_id, function (response) {
            if (response.responseCode == 1) {
                $(".drawer-body").html(response.html);
                const drawerElement = document.querySelector("#kt_drawer_chat");
                if (drawerElement) {
    ...
    $(document).on("submit", "#experience_section_add_descriptions_form", function (e) {
        console.log("e:", e);
        e.preventDefault();
        var project_id = $(this).attr('data-experience-id');
        $("#msg_box").html("");
        $(".btn-save").attr("disabled", "disabled");
        var act_val = $("#act").val();
        var eid = $("#eid").val();
        var params = act_val;
        var dynamic_url = "/resume/experience-sections/descriptions/add/" + project_id;
        // if (eid != "") {
    ...
    $(document).on('change', 'input[data-type="file"]', function(event) {
        ...
        $('div[data-type="company-logo"]').css('background-image', `url(${imageUrl})`);
    });
...
$(document).on('blur', '.sort_number_input', function(){
    ...
    var row_id = $(this).attr('data-experience-id');
    $.ajax({
        url: admin_url + "/resume/experience-sections/sorting/" + row_id + "/" + sort_number,
        method: "GET",
        success: function (data) {
            var obj = JSON.parse(data);
            if (obj.responseCode == 1) {
